...
import {
  IPlanSourceInfo, IKeomPreparePlan, IKeomPreparePlanBadPaths,
...
//region before, after
  before(async function () {
    ...
    comptroller = KeomHelper.getComptroller(signer, ZkevmAddresses.ZEROVIX_COMPTROLLER);
    priceOracle = await KeomHelper.getPriceOracle(signer, ZkevmAddresses.ZEROVIX_COMPTROLLER);
  ...
  describe("getConversionPlan", () => {
    ...
    async function getConversionPlan(p: IKeomPreparePlan): Promise<IResults> {
      ...
      const {
        ...
        sourceInfo,
        gasUsed
      } = await KeomPlatformAdapterUtils.getConversionPlan(signer, comptroller, priceOracle, p, pa, poolAdapterTemplate);
  ...
  describe("getBorrowRateAfterBorrow", () => {
    describe("Good paths", () => {
      async function makeTest(p: IPredictBrParams): Promise<IPredictBrResults> {
        const collateralToken = IMToken__factory.connect(KeomUtilsZkevm.getCToken(p.collateralAsset), signer);
        const borrowToken = IMToken__factory.connect(KeomUtilsZkevm.getCToken(p.borrowAsset), signer);
