...
import {
  Aave3PlatformAdapter, AaveTwoPlatformAdapter, Accountant, Accountant__factory,
  BorrowManager,
...
import {
  BorrowRepayCases, IAccountantStatus,
...
/** Ensure that all repay/borrow operations are correctly registered in the Accountant */
describe("AccountantCaseTest", () => {
//region Data types
  ...
  NETWORKS.forEach(network => {
    describe(`${network.networkId}`, function () {
      before(async function () {
        ...
        accountant = await Accountant__factory.connect(await converterController.accountant(), signer);
      });
            ...
            if (assetPair.singleParams) {
              describe(`${assetPair.collateralAssetName} : ${assetPair.borrowAssetName}`, function () {
                ...
                describe("first borrow", function () {
                  ...
                  async function makeBorrowTest(): Promise<IAccountantStatus> {
                    return BorrowRepayCases.borrowRepayPairsSingleBlockAccountant(
                      signer,
                      {
                  ...
                  it("should add borrow action to Accountant", async () => {
                  ...
                  describe("partial repay", function () {
                    ...
                    REPAY_PARTS.forEach(repayPart => {
                      describe(`repay part ${repayPart / 100_000}`, function () {
                        ...
                        let ret2: IAccountantStatus;
                        before(async function () {
                          snapshotLevel2 = await TimeUtils.snapshot();
                          ret2 = await BorrowRepayCases.borrowRepayPairsSingleBlockAccountant(
                            signer,
                            {
                              tetuConverter: ITetuConverter__factory.connect(await converterController.tetuConverter(), signer),
                              user: userEmulator,
                        ...
                        it("should add repay action to Accountant", async () => {
                  ...
                  describe("second borrow", function () {
                    ...
                    async function makeSecondBorrowTest(): Promise<IAccountantStatus> {
                      return BorrowRepayCases.borrowRepayPairsSingleBlockAccountant(
                        signer,
                        {
                          tetuConverter: ITetuConverter__factory.connect(await converterController.tetuConverter(), signer),
                    ...
                    it("should add second borrow action to Accountant", async () => {
                      expect(ret2.actions.length).eq(2);
                      expect(ret2.actions[0].actionKind).eq(AppConstants.ACTION_KIND_BORROW_0);
