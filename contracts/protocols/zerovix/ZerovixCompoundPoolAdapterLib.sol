...
///      They also don't generate exceptions, so we should check result balances after each call.
///      This library is intended to use with ERC20 tokens (they support IZerovixToken)
///      Native token supports IZerovixTokenNative (with different signatures of borrow, mint, etc)
///      so it will require different implementation.
library ZerovixCompoundPoolAdapterLib {
  ...
  struct State {
    ...
    ICompoundComptrollerBaseV2Zerovix comptroller;
  ...
  function initialize(
    ...
    dest.comptroller = ICompoundComptrollerBaseV2Zerovix(comptroller_);
  ...
  function updateStatus(State storage state) internal {
    ...
    IZerovixToken(state.borrowCToken).borrowBalanceCurrent(address(this));
    IZerovixToken(state.collateralCToken).exchangeRateCurrent();
  }
  ...
  function repay(
    ...
    IZerovixToken(v.cTokenBorrow).borrowBalanceCurrent(address(this));
    ...
    // infinity approve
    IZerovixToken(v.cTokenBorrow).repayBorrow(amountToRepay_);
    // todo check results

    // withdraw the collateral
    v.balanceCollateralAssetBeforeRedeem = _getBalance(f_, v.assetCollateral);
    IZerovixToken(v.cTokenCollateral).redeem(v.collateralTokensToWithdraw);
    // todo check results
  ...
  function repayToRebalance(
    ...
    ICompoundComptrollerBaseV2Zerovix comptroller = state.comptroller;
    uint tokenBalanceBefore;
    ...
    if (isCollateral_) {
      ...
      // transfer borrow amount back to the pool
      IZerovixToken(cTokenBorrow).repayBorrow(amountIn_);
      // todo check results
  ...
  function _validateHealthStatusAfterBorrow(
    ...
    ICompoundComptrollerBaseV2Zerovix comptroller_,
  ...
  function _initAccountData(address cTokenCollateral, address cTokenBorrow, AccountData memory dest) internal view {
    ...
    (error, dest.collateralTokenBalance,, dest.exchangeRateCollateral) = IZerovixToken(cTokenCollateral).getAccountSnapshot(address(this));
    require(error == 0, string(abi.encodePacked(AppErrors.CTOKEN_GET_ACCOUNT_SNAPSHOT_FAILED, Strings.toString(error))));

    (error,, dest.borrowBalance,) = IZerovixToken(cTokenBorrow).getAccountSnapshot(address(this));
    require(error == 0, string(abi.encodePacked(AppErrors.CTOKEN_GET_ACCOUNT_SNAPSHOT_FAILED, Strings.toString(error))));
  }
  ...
  function _getCollateralFactor(
    ...
    (, , collateralFactor) = comptroller_.markets(cTokenCollateral_);
  }
