...
contract KeomPlatformAdapter is IPlatformAdapter, ITokenAddressProvider {
  ...
  function platformKind() external pure returns (AppDataTypes.LendingPlatformKinds) {
    return AppDataTypes.LendingPlatformKinds.KEOM_8;
  }
  ...
  /// @dev Keom's comptroller has some methods with not-standard signature:
  function getMarketsInfo(address cTokenCollateral_, address cTokenBorrow_) public view returns (
    uint ltv18,
    uint liquidityThreshold18
  ) {
    IKeomComptroller _comptroller = IKeomComptroller(payable(address(_state.comptroller)));
    (, bool borrowPaused, , bool repayPaused) = _comptroller.guardianPaused(cTokenBorrow_);
    (bool mintPaused, , bool redeemPaused,) = _comptroller.guardianPaused(cTokenCollateral_);
    if (!borrowPaused && !mintPaused && !repayPaused && !redeemPaused) {
      bool isListed;
