...
import "../../integrations/zerovix/IOMatic.sol";
import "../../integrations/IWmatic.sol";


/// @notice Implementation of IPoolAdapter for 0vix
...
contract ZerovixPoolAdapter is IPoolAdapter, IPoolAdapterInitializerWithAP, Initializable {
  ...
  I0vixComptroller private _comptroller;
  ...
  initializer
  {
    ...
    _comptroller = I0vixComptroller(comptroller_);
  ...
  function updateStatus() external override {
    ...
    IOToken(borrowCToken).borrowBalanceCurrent(address(this));
    IOToken(collateralCToken).exchangeRateCurrent();
  }
  ...
  function borrow(
    ...
    uint error;
    I0vixComptroller comptroller = _comptroller;
    ...
    // make borrow
    {
      ...
      error = IOErc20(cTokenBorrow).borrow(borrowAmount_);
  ...
  function _supply(
    ...
    if (_isMatic(assetCollateral_)) {
      ...
      IOMatic(payable(cTokenCollateral_)).mint{value : collateralAmount_}();
    } else {
      // replaced by infinity approve: IERC20(assetCollateral_).approve(cTokenCollateral_, collateralAmount_);
      uint error = IOErc20(cTokenCollateral_).mint(collateralAmount_);
  ...
  function repay(
    ...
    IOToken(vars.cTokenBorrow).borrowBalanceCurrent(address(this));
    ...
    if (_isMatic(vars.assetBorrow)) {
      ...
      IOMatic(payable(vars.cTokenBorrow)).repayBorrow{value : amountToRepay_}();
    } else {
      // replaced by infinity approve: IERC20(assetBorrow).approve(cTokenBorrow, amountToRepay_);
      vars.error = IOErc20(vars.cTokenBorrow).repayBorrow(amountToRepay_);
      require(vars.error == 0, AppErrors.REPAY_FAILED);
    }

    // withdraw the collateral
    uint balanceCollateralAssetBeforeRedeem = _getBalance(vars.assetCollateral);
    vars.error = IOErc20(vars.cTokenCollateral).redeem(collateralTokensToWithdraw);
  ...
  function _getCollateralTokensToRedeem(
    ...
    (uint error, uint tokenBalance,,) = IOToken(cTokenCollateral_).getAccountSnapshot(address(this));
    require(error == 0, AppErrors.CTOKEN_GET_ACCOUNT_SNAPSHOT_FAILED);

    (uint error2,, uint borrowBalance,) = IOToken(cTokenBorrow_).getAccountSnapshot(address(this));
  ...
  function repayToRebalance(
    ...
    if (isCollateral_) {
      ...
      if (_isMatic(assetBorrow)) {
        ...
        IOMatic(payable(cTokenBorrow)).repayBorrow{value : amount_}();
      } else {
        // replaced by infinity approve: IERC20(assetBorrow).approve(cTokenBorrow, amount_);
        error = IOErc20(cTokenBorrow).repayBorrow(amount_);
  ...
  function getCollateralAmountToReturn(uint amountToRepay_, bool closePosition_) external view override returns (uint) {
    ...
    (uint error,,, uint cExchangeRateMantissa) = IOToken(cTokenCollateral).getAccountSnapshot(address(this));
    require(error == 0, AppErrors.CTOKEN_GET_ACCOUNT_SNAPSHOT_FAILED);
  ...
  function _getStatus(address cTokenCollateral, address cTokenBorrow) internal view returns (
    ...
    (error, tokenBalanceOut,, cExchangeRateMantissa) = IOToken(cTokenCollateral)
      .getAccountSnapshot(address(this));
    require(error == 0, AppErrors.CTOKEN_GET_ACCOUNT_SNAPSHOT_FAILED);

    (error,, borrowBalanceOut,) = IOToken(cTokenBorrow).getAccountSnapshot(address(this));
    require(error == 0, AppErrors.CTOKEN_GET_ACCOUNT_SNAPSHOT_FAILED);

    IPriceOracle priceOracle = IPriceOracle(_comptroller.oracle());
    uint priceCollateral = priceOracle.getUnderlyingPrice(IOToken(cTokenCollateral));

    collateralBaseOut = (priceCollateral * cExchangeRateMantissa / 10**18) * tokenBalanceOut / 10**18;
    borrowBaseOut = priceOracle.getUnderlyingPrice(IOToken(cTokenBorrow)) * borrowBalanceOut / 10**18;
...
//    return int(IOToken(borrowCToken).borrowRatePerBlock() * controller.blocksPerDay() * 365 * 100);
//  }
  ...
  internal view returns (
    ...
    I0vixComptroller.Market memory market = _comptroller.markets(cTokenCollateral_);
//    (,uint collateralFactor,) = _comptroller.markets(cTokenCollateral_);

    sumCollateralSafe = market.collateralFactorMantissa * sumCollateral_ / 10**18;
